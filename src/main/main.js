...
// â”€â”€ Ollama auto-start â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ipcMain.handle('load-emails', async () => {
  ...
  try {
    ...
    console.log(`âœ… Loaded ${emails.length} emails from email.txt`);
...
ipcMain.handle('add-email', async (event, emailData) => {
  ...
  try {
    ...
    console.log(`âœ… Added new email: ${emailData.email}${recovery ? ' with recovery' : ''}`);
...
ipcMain.handle('add-emails-bulk', async (event, emailsArray) => {
  ...
  try {
    ...
    console.log(`âœ… Added ${emailsArray.length} emails in bulk`);
...
// ============================
// CONSISTENCY TRACKER IPC HANDLERS
// ============================
...
ipcMain.handle('save-consistency-data', async (event, data) => {
  try {
    ...
    console.log('âœ… Consistency data saved successfully');
    ...
    if (fs.existsSync(backupPath)) {
      try {
        ...
        console.log('âš ï¸ Restored from backup after save failure');
      } catch (restoreError) {
...
ipcMain.handle('save-distro-releases', async (event, releases) => {
  try {
    ...
    fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
    console.log('âœ… Distro releases saved successfully');
...
const automationConfigPath = path.join(APP_ROOT, 'automation', 'config');
      ...
      const defaultSettings = {
        templates: {
          template1: {
            ...
            description: "ðŸ’°PURCHASE on instagram @sheloveslvmh\n\nâ€¢ You must credit with (PROD. LVMH) in the title",
  ...
  try {
    ...
    const defaultConfig = {
      ...
      metadataTemplate: {
        ...
        description: 'ðŸ’°PURCHASE on instagram @prodvince\n\nâ€¢ You must credit with (PROD. VINCE) in the title\nâ€¢ You must purchase a lease to upload your track to streaming platforms like Spotify, Apple Music, etc.\n\nThis beat is free for NON PROFIT USE ONLY',
        tags: ['beats'],
        descriptionConditions: {
          default: {
            text: 'ðŸ’°PURCHASE on instagram @prodvince\n\nThis beat is free for NON PROFIT USE ONLY',
            tags: 'beats'
          }
        }
      }
...
ipcMain.handle('start-automation-server', async () => {
  try {
    ...
    if (!fs.existsSync(mainScript)) {
      return { success: false, error: 'KhÃ´ng tÃ¬m tháº¥y main-multi.js' };
    }

    // Check if server is already running
    if (automationServerProcess && !automationServerProcess.killed) {
      return { success: true, message: 'Server Ä‘ang cháº¡y' };
    }
    ...
    return { success: true, message: 'Server Ä‘Ã£ khá»Ÿi Ä‘á»™ng' };
  } catch (error) {
    return { success: false, error: error.message };
  }
});
...
ipcMain.handle('stop-automation-server', async () => {
  try {
    if (automationServerProcess && !automationServerProcess.killed) {
      ...
      automationServerProcess = null;
      return { success: true, message: 'Server Ä‘Ã£ dá»«ng' };
    }
    ...
    return new Promise((resolve) => {
      if (process.platform === 'win32') {
        exec('netstat -ano | findstr :9000', (error, stdout) => {
          ...
          resolve({ success: true, message: 'Server Ä‘Ã£ dá»«ng' });
        });
      } else {
...
- Read EVERY character exactly as it appears â€” never autocorrect or normalize.
...
- The letters u, i, y can look similar at small sizes â€” look carefully at each one.
- Do NOT add or remove characters from what you see.

Extract ONLY:
1. The display name at the top (copy exactly â€” count repeated chars carefully)
...
{"name":"","instagram":"","email":"","beats_bought":[],"notes":"","confidence":"high|medium|low"}`;
    ...
    // Use Ollama native /api/generate â€” images are passed as raw base64 array (no data URL prefix)
    ...
    return new Promise((resolve) => {
      const options = {
        hostname: '127.0.0.1',  // explicit IPv4 â€” avoids Windows localhostâ†’::1 IPv6 issue
        port: 11434,
        path: '/api/generate',
        method: 'POST',
      ...
      req.setTimeout(120000, () => { req.destroy(); resolve({ success: false, error: 'Vision model timed out (120s). Try again â€” first run loads the model.' }); });
      req.write(postData);
      req.end();
    });
  } catch (e) {
...
// AI AGENT â€” Ollama function-calling loop
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function executeAgentTool(toolName, toolArgs) {
  ...
  switch (toolName) {
    ...
    case 'rename_pack': {
      ...
      writeBeats(data);
      return `Renamed "${old}" â†’ "${toolArgs.new_name}"`;
    }
...
ipcMain.handle('ai-command', async (event, { message, history }) => {
  try {
    ...
    const messages = [
      {
        role: 'system',
        content: `You are an AI assistant built into Beats Management Studio â€” a music producer app for managing beat packs, customers, and marketing campaigns. You control the app directly using the available tools. Be concise and action-oriented. After completing tasks, briefly summarize what you did.`
      },
      ...(history || []),
...
// --- Easy DOM Grabber: clipboard handler ---
ipcMain.handle('edg-copy-to-clipboard', async (_event, text) => {
  clipboard.writeText(text);
  return true;
});

// --- Easy DOM Grabber: toggle from main process ---
...
// --- Easy DOM Grabber: injection code ---
